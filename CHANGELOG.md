...
- **视频转发异常降级**：修复 AstrBot / aiocqhttp / NapCat 组合下，部分 X 视频在 QQ NT `sendMsg` 阶段超时或返回 `rich media transfer failed` 时，插件长时间反复尝试发送导致体验卡顿的问题。现在识别到 QQ 富媒体传输拒绝后会直接降级为视频直链，避免继续触发文件上传重试。
- **本地视频组件构造兼容**：视频缓存文件发送改用 AstrBot 当前推荐的 `Video.fromFileSystem()`，与图片的 `Image.fromFileSystem()` 路径处理保持一致，减少平台更新后本地文件路径格式不兼容的风险。

### Changed

- **aiocqhttp 视频候选策略调整**：在 QQ / OneBot 场景下优先尝试最小 MP4 变体，减少高分辨率视频上传等待；普通发送错误仍保留文件上传兜底，明确的 QQ 富媒体风控错误则直接输出直链。
...
- 增加订阅关注功能，可以添加指定用户为关注用户，自动化拉取关注用户最新发送的推文，并自动发送到指定群聊。
